...
class GroundedScan(object):
    ...
    @classmethod
    def load_dataset_from_file(cls, file_path: str, save_directory: str, k=5, upsample_isolated=100,
                               isolate_examples_with="cautiously"):
        with open(file_path, 'r') as infile:
            ...
            for split, examples in all_data["examples"].items():
                for i, example in enumerate(examples):
                    if (split == "train" or split == "dev") and "cautiously" in example["command"]:
                        adverb_examples.append(example)
                    else:
                        if split == "dev" or split == "test":
                            if example["manner"] not in ["cautiously", "hesitantly", "while spinning", "while zigzagging", ""]:
                                continue
                        dataset._data_pairs[split].append(example)
                        dataset.update_data_statistics(example, split)
            random.shuffle(adverb_examples)
            if split == "train":
                print(len(adverb_examples))
            for example in adverb_examples[:k]:
                for j in range(upsample_isolated):
    ...
    def error_analysis(self, predictions_file: str, output_file: str, save_directory: str):
        ...
        all_accuracies = []
        exact_matches = []
        exact_matches_below_training_length = []
        workbook = Workbook()
        with open(predictions_file, 'r') as infile:
            ...
            for predicted_example in data:
                ...
                if example_information["target_length"] < max(self._data_statistics["train"]["target_length"].keys()):
                    exact_matches_below_training_length.append(exact_match)
        ...
        with open(output_file, 'w') as outfile:
            ...
            exact_matches_counter_below = Counter(exact_matches_below_training_length)
            outfile.write(" Max target length seen in training: {}\n".format(
                max(self._data_statistics["train"]["target_length"].keys())))
            outfile.write(" Num. exact matches below length: {}\n".format(exact_matches_counter_below[True]))
            outfile.write(" Num not exact matches below length: {}\n".format(exact_matches_counter_below[False]))
            outfile.write(" Exact match percentage below length: {}\n\n".format(
                exact_matches_counter_below[True] / (
                            exact_matches_counter_below[True] + exact_matches_counter_below[False])))
