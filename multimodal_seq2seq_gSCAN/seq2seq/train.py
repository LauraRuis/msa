...
def train(data_path: str, data_directory: str, generate_vocabularies: bool, input_vocab_path: str,
          ...
          weight_target_loss: float, attention_type: str, k: int, simplified_objective: bool,
    ...
    training_set = GroundedScanDataset(data_path, output_directory, split="train",
                                       input_vocabulary_file=input_vocab_path,
                                       target_vocabulary_file=target_vocab_path,
                                       generate_vocabulary=generate_vocabularies, k=k,
                                       simplified_objective=simplified_objective)
    ...
    test_set = GroundedScanDataset(data_path, output_directory, split="dev",  # TODO: use dev set here
                                   input_vocabulary_file=input_vocab_path,
                                   target_vocabulary_file=target_vocab_path, generate_vocabulary=False, k=k,
                                   simplified_objective=simplified_objective)
    ...
    while training_iteration < max_training_iterations:
        ...
        for (input_batch, input_lengths, _, situation_batch, _, target_batch,
             target_lengths, agent_positions, target_positions) in training_set.get_data_iterator(
                batch_size=training_batch_size, max_examples=max_training_examples,
                simple_situation_representation=simple_situation_representation, shuffle=True):
            is_best = False
            model.train()
            ...
            if training_iteration % evaluate_every == 0:
                with torch.no_grad():
                    ...
                    accuracy, exact_match, target_accuracy = evaluate(
                        test_set.get_data_iterator(batch_size=1, max_examples=None, shuffle=False), model=model,
