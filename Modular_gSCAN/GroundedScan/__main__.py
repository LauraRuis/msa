...
def main():
    ...
    parser.add_argument('--num_train_examples_per_train_adverb', default=None,
    ...
    if flags["num_train_examples_per_train_adverb"] == "None" or flags["num_train_examples_per_train_adverb"] is None:
        flags["num_train_examples_per_train_adverb"] = None
    else:
        flags["num_train_examples_per_train_adverb"] = int(flags["num_train_examples_per_train_adverb"])
    ...
    if flags['mode'] == 'generate':
        ...
        for file in files:
            logger.info("Performing position analysis on file with predictions: {}".format(
                file))
            split = file.split("split_")[1].split(".json")[0]
            grounded_scan = GroundedScan.load_dataset_from_file(flags["load_dataset_from"],
                                                                splits=[split],
                                                                save_directory=flags["output_directory"],
                                                                k=5,
                                                                upsample_isolated=1,
                                                                isolate_adverb_types=flags["isolate_adverb_types"],
                                                                only_keep_adverbs=flags["only_keep_adverbs"],
        ...
        adverb_world.generate_adverb_challenge(
            ...
            num_resampling=flags["num_resampling"],
            visualize_per_split=flags["visualize_per_split"],
            grid_size=6, make_dev_set=True, use_gscan_adverbs=True,
            populate_all_adverbs=True)
