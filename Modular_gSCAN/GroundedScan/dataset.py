...
class GroundedScan(object):
    ...
    def get_examples_with_image(self, split="train", simple_situation_representation=False, adverb_inputs=False,
        ...
        for idx in indices:
            ...
            if not adverb_inputs:
                if example["manner"] == "cautiously" or example["manner"] == "hesitantly":
                    example["type_adverb"] = "nonmovement_first_person"
                elif example["manner"] == "while spinning":
                    example["type_adverb"] = "nonmovement_direction"
                elif example["manner"] == "while zigzagging":
                    example["type_adverb"] = "movement_rewrite"
                else:
                    example["type_adverb"] = None

                yield {"input_command": command, "input_meaning": meaning, "adverb": example["manner"],
                       "type_adverb": example["type_adverb"], "verb_in_command": example["verb_in_command"],
    ...
    def update_data_statistics(self, data_example, split="train"):
        ...
        # adverb stats
        if "type_adverb" in data_example:
            self._data_statistics[split]["type_adverbs"][data_example["type_adverb"]] += 1
    ...
    @classmethod
    def load_dataset_from_file(cls, file_path: str, save_directory: str, splits: List[str], k=None,
                               isolate_examples_with="cautiously", upsample_isolated=1,
                               isolate_adverb_types=None, only_keep_adverbs=None, dev_k=0, seed=1):
        if upsample_isolated != 1 and k is None:
            raise ValueError("If upsample_isolated set to > 1 also choose a k.")
        if isolate_adverb_types:
            assert isinstance(isolate_adverb_types, list), "Please provide list of strings for arg isolate_adverb_types"
        else:
            isolate_adverb_types = set()
        if only_keep_adverbs:
            assert isinstance(only_keep_adverbs,
                              list), "Please provide list of strings for arg only_keep_adverbs"
        with open(file_path, 'r') as infile:
            all_data = json.load(infile)
            percentage_train = all_data.get("percentage_train")
            if not percentage_train:
                percentage_train = 0.8
            if "adverb_splits" in all_data:
                adverb_splits = all_data["adverb_splits"].split(";")
            else:
                adverb_splits = []
            dataset = cls(all_data["intransitive_verbs"], all_data["transitive_verbs"], all_data["adverbs"],
                          ...
                          percentage_train=percentage_train, adverb_splits=adverb_splits,
                          max_recursion=all_data["max_recursion"], sample_vocabulary='load', seed=seed)
            dataset._data_pairs["isolated_examples"] = []
            dataset._data_pairs["isolated_adverb_types"] = []
            for split in splits:
                examples = all_data["examples"][split]
                for i, example in enumerate(examples):
                    if i > 3550000:
                        break
                    if not "type_adverb" in example:
                        if example["manner"] == "cautiously" or example["manner"] == "hesitantly":
                            example["type_adverb"] = "nonmovement_first_person"
                        elif example["manner"] == "while spinning":
                            example["type_adverb"] = "nonmovement_direction"
                            ...
                            example["type_adverb"] = None
                    if isolate_examples_with in example["command"] and (
                            split == "train" or split == "dev" or split == "test"):
                        dataset._data_pairs["isolated_examples"].append(example)
                    elif example["type_adverb"] in isolate_adverb_types and (split == "train" or split == "dev" or split == "test") \
                            and example["manner"] not in ["cautiously", "hesitantly", "while spinning",
                                                          "while zigzagging", ""]:
                        dataset._data_pairs["isolated_adverb_types"].append(example)
                    else:
                        if only_keep_adverbs:
                            if (example["manner"] in ["cautiously", "hesitantly", "while spinning", "while zigzagging", ""]
                                or example["manner"] in only_keep_adverbs) and (
                                    split == "train" or split == "dev" or split == "test" or split == "adverb_1" or split == "adverb_2"):
                            ...
                            if split == "dev" or split == "test":
                                if example["manner"] not in ["cautiously", "hesitantly", "while spinning",
                                                             "while zigzagging", ""]:
                                    continue
                            dataset._data_pairs[split].append(example)
                            dataset.update_data_statistics(example, split)
            ...
            if k:
                train_k = k - dev_k
                k_shot_examples = isolated_examples[:train_k] * upsample_isolated
                dev_examples = isolated_examples[train_k:train_k + dev_k]
            else:
                dev_k = len(isolated_examples) // 2
                train_k = len(isolated_examples) - dev_k
                k_shot_examples = isolated_examples[:train_k]
                dev_examples = isolated_examples[train_k:train_k + dev_k]
            ...
            for i, dev_example in enumerate(dev_examples):
                dataset._data_pairs["dev"].append(dev_example)
                dataset.update_data_statistics(dev_example, "dev")
            return dataset
    ...
    def error_analysis(self, predictions_file: str, output_file: str, save_directory: str):
        ...
        with open(predictions_file, 'r') as infile:
            ...
            for predicted_example in data:
                ...
                derivation = self.parse_derivation_repr(predicted_example["derivation"])
                arguments = []
    ...
    def generate_adverb_world_states(self, held_out_adverbs: List[str], example_parser: Callable,
        ...
        while adverbs_left:
            ...
            if adverb_types(self._vocabulary.translate_word(adverb)) == "movement_rewrite":
                ...
                for target_size, target_color, target_shape in possible_target_objects:
                    ...
                    for i, relevant_situation in enumerate(relevant_situations):
                        ...
                        if self._vocabulary.translate_word(adverb) in [
                            "while spinning", "cautiously", "hesitantly", "while zigzagging"]:
                            assert ','.join(old_target_commands) == ','.join(targets_example["target_commands"]), \
                            "Failed for adverb %s with \nold: %s, \nnew: %s" % (
                                adverb, ','.join(old_target_commands), ','.join(targets_example["target_commands"]))
                        if len(splits) == 0:
                            splits = ["train"]
                        elif len(splits) > 1:
                            dropped_examples += 1
