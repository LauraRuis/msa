...
def test_gscan_examples(dataset, generate_examples=False, split="train"):
    start = time.time()
    current_situation = dataset._world.get_current_situation()
    current_mission = dataset._world.mission
    ...
    indices = [i for i in range(len(dataset._data_pairs[split]))]
    ...
    for example_idx in indices[:num_test_examples]:
        example = dataset._data_pairs[split][example_idx]
    ...
    for verb, adverb_counts in verb_adverb.items():
        for adverb, count in adverb_counts.items():
            logger.info("Tested %d examples for verb %s with adverb %s" % (count, verb, adverb))
    dataset.initialize_world(current_situation, mission=current_mission)
    end = time.time()
...
def test_generate_adverb_challenge():
    ...
    adverb_world.generate_adverb_challenge(num_extra_training_adverbs=40,
                                           ...
                                           visualize_per_split=10,
                                           populate_all_adverbs=False,
                                           with_nonterminal=None)
...
def test_compare_gscan_lsystems():
    """
    Test whether gSCAN systems that are the same as the original gSCAN adverbs are correctly recognized by the
    __eq__ method in LSystem.
    """
    start = time.time()
    adverb_world = dsl.AdverbWorld(seed=1, save_directory=TEST_DIRECTORY)

    _hesitantly = dsl.LSystem()
    meta_grammar = dsl.MetaGrammar()
    _hesitantly.add_rule(meta_grammar.get_rule(lhs_str="Push",
                                                    rhs_str="Push ACTION"))
    _hesitantly.add_rule(meta_grammar.get_rule(lhs_str="Pull",
                                                    rhs_str="Pull ACTION"))
    _hesitantly.add_rule(meta_grammar.get_rule(lhs_str="Walk",
                                               rhs_str="Walk ACTION"))
    _hesitantly.add_rule(meta_grammar.get_rule(lhs_str="ACTION",
                                                    rhs_str="Stay"),
                              terminal_rule=True)
    _hesitantly.finish_l_system()
    _hesitantly = dsl.apply_recursion(_hesitantly, max_recursion=1)

    _cautiously = dsl.LSystem()
    _cautiously.add_rule(meta_grammar.get_rule(lhs_str="Pull",
                                               rhs_str="ACTION Pull"))
    _cautiously.add_rule(meta_grammar.get_rule(lhs_str="Walk",
                                                    rhs_str="ACTION Walk"))
    _cautiously.add_rule(meta_grammar.get_rule(lhs_str="Push",
                                                    rhs_str="ACTION Push"))
    _cautiously.add_rule(meta_grammar.get_rule(lhs_str="ACTION",
                                               rhs_str="Tl"),
                         terminal_rule=True)
    _cautiously.add_rule(meta_grammar.get_rule(lhs_str="{ACTION}ACTION ACTION{ACTION}",
                                               rhs_str="Tr Tr"),
                         terminal_rule=True)
    _cautiously.finish_l_system()
    _cautiously = dsl.apply_recursion(_cautiously, max_recursion=4)


    cautiously_push = dsl.LRule(lhs=dsl.Lhs(_cautiously.convert_rule_side("Push", meta_grammar)),
                                rhs=dsl.Rhs(_cautiously.convert_rule_side("Tl Tr Tr Tl Push", meta_grammar)))
    cautiously_walk = dsl.LRule(lhs=dsl.Lhs(_cautiously.convert_rule_side("Walk", meta_grammar)),
                                rhs=dsl.Rhs(_cautiously.convert_rule_side("Tl Tr Tr Tl Walk", meta_grammar)))
    cautiously_pull = dsl.LRule(lhs=dsl.Lhs(_cautiously.convert_rule_side("Pull", meta_grammar)),
                                rhs=dsl.Rhs(_cautiously.convert_rule_side("Tl Tr Tr Tl Pull", meta_grammar)))
    _cautiously_two = dsl.LSystem()
    _cautiously_two.add_rule(cautiously_push, terminal_rule=False)
    _cautiously_two.add_rule(cautiously_walk, terminal_rule=False)
    _cautiously_two.add_rule(cautiously_pull, terminal_rule=False)
    _cautiously_two.finish_l_system()

    _while_spinning = dsl.LSystem()
    _while_spinning.add_rule(meta_grammar.get_rule(lhs_str="East",
                                                   rhs_str="ACTION East"))
    _while_spinning.add_rule(meta_grammar.get_rule(lhs_str="West",
                                                   rhs_str="ACTION West"))
    _while_spinning.add_rule(meta_grammar.get_rule(lhs_str="Pull",
                                                   rhs_str="ACTION Pull"))
    _while_spinning.add_rule(meta_grammar.get_rule(lhs_str="North",
                                                   rhs_str="ACTION North"))
    _while_spinning.add_rule(meta_grammar.get_rule(lhs_str="South",
                                                   rhs_str="ACTION South"))
    _while_spinning.add_rule(meta_grammar.get_rule(lhs_str="Push",
                                                   rhs_str="ACTION Push"))
    _while_spinning.add_rule(meta_grammar.get_rule(lhs_str="ACTION",
                                                   rhs_str="Tl"),
                             terminal_rule=True)
    _while_spinning.finish_l_system()
    _while_spinning = dsl.apply_recursion(_while_spinning, max_recursion=4)

    _while_zigzagging = dsl.LSystem()
    _while_zigzagging.add_rule(meta_grammar.get_rule(lhs_str="East South",
                                                     rhs_str="South East"))
    _while_zigzagging.add_rule(meta_grammar.get_rule(lhs_str="East North",
                                                     rhs_str="North East"))
    _while_zigzagging.add_rule(meta_grammar.get_rule(lhs_str="West South",
                                                     rhs_str="South West"))
    _while_zigzagging.add_rule(meta_grammar.get_rule(lhs_str="West North",
                                                     rhs_str="North West"))
    _while_zigzagging.finish_l_system()

    gscan_l_systems = [_hesitantly, _cautiously, _while_spinning, _while_zigzagging, _cautiously_two]
    gscan_l_systems_two = [adverb_world.get_adverb_program(adverb) for adverb in adverb_world._adverbs]

    # Compare gSCAN systems to newly created gSCAN LSystems with different ordering of rules, but equivalent
    for gscan_l_system in gscan_l_systems:
        assert adverb_world.is_gscan_lsystem(gscan_l_system), "test_compare_gscan_lsystems FAILED when comparing "\
                                                              "same LSystem, but different ordering of rules."

    # Compare gSCAN systems to themselves
    for gscan_l_system in gscan_l_systems_two:
        assert adverb_world.is_gscan_lsystem(gscan_l_system), "test_compare_gscan_lsystems FAILED when comparing same "\
                                                              "LSystem."

    end = time.time()
    logger.info("test_compare_gscan_lsystems PASSED in {} seconds".format(end - start))
...
def run_all_tests():
    ...
    # test_gscan_examples(TEST_DATASET_2, generate_examples=True)
    # test_grounded_scan = GroundedScan.load_dataset_from_file(os.path.join("data/compositional_splits", "dataset.txt"),
    #                                                          TEST_DIRECTORY)
    # test_gscan_examples(test_grounded_scan, generate_examples=False, split="train")
    # test_adverb_sampling(TEST_DATASET_2)
    # test_remove_out_of_grid(TEST_DATASET_2)
    # test_save_and_load_adverb_challenge()
    # test_compare_gscan_lsystems()
