...
class LSystem(object):
    ...
    def are_the_same_rules(self, rule_dict_one, rule_dict_two):
        for lhs_length, rules in rule_dict_one.items():
            if lhs_length not in rule_dict_two:
                return False
            for lhs, this_rule in rules.items():
                if lhs not in rule_dict_two[lhs_length]:
                    return False
                that_rule = rule_dict_two[lhs_length][lhs]
                if str(this_rule) != str(that_rule):
                    return False
        return True

    def __eq__(self, l_system: "LSystem"):
        this_representation = self._rewrite_rules
        that_representation = l_system._rewrite_rules
        that_representation_contained_in_this = self.are_the_same_rules(that_representation, this_representation)
        if not that_representation_contained_in_this:
            return False
        this_representation_contained_in_that = self.are_the_same_rules(this_representation, that_representation)
        if not this_representation_contained_in_that:
            return False
        this_terminals = self._terminal_rules
        that_terminals = l_system._terminal_rules
        this_terminals_contained_in_that = self.are_the_same_rules(this_terminals, that_terminals)
        if not this_terminals_contained_in_that:
            return False
        that_terminals_contained_in_this = self.are_the_same_rules(that_terminals, this_terminals)
        if not that_terminals_contained_in_this:
            return False
        return True
...
class AdverbWorld(object):
    ...
    def save_gscan_statistics(self, dev_set: bool):
        ...
        splits = ["train", "test", "visual", "situational_1", "situational_2", "contextual", "adverb_1", "adverb_2",
                  "visual_easier"] + held_out_splits
        if dev_set:
            splits += ["dev"]
        for split in splits:
    ...
    @classmethod
    def load_adverb_challenge(cls, file_path: str, save_directory: str, isolate_examples_with="cautiously"):
    ...
    def generate_adverb_challenge(self, num_extra_training_adverbs: int,
        ...
        :param with_nonterminal: which nonterminal has to be in all derivations
        (e.g., RB for examples with adverbs only, None for generating all derivations / type examples)
        """
        total_num_adverbs = num_extra_training_adverbs + num_extra_testing_adverbs
        if total_num_adverbs > 0 and not populate_all_adverbs:
            self.populate_adverbs(max_per_type=total_num_adverbs)
        elif populate_all_adverbs:
            self.populate_adverbs()
        ...
        if not num_train_examples_per_train_adverb:
            self._gscan_dataset.generate_all_adverb_world_states(
                ...
                held_out_adverbs=held_out_adverbs,
                with_nonterminal=with_nonterminal
            )
        else:
