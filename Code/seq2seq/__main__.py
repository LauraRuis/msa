...
parser.add_argument("--max_training_iterations", type=int, default=200000)
...
parser.add_argument("--simplified_objective", type=str, default="no")
...
def main(flags):
    ...
    if flags["simplified_objective"] == "no":
        flags["simplified_objective"] = False
    elif flags["simplified_objective"] == "yes":
        flags["simplified_objective"] = True
    else:
        raise ValueError("Unrecognized value for --simplified_obective=%s. Options: ['yes', 'no']"
                         % flags["simplified_objective"])
    ...
    if flags["mode"] == "train":
        ...
        assert os.path.exists(os.path.join(flags["output_directory"], flags["input_vocab_path"])) and os.path.exists(
            os.path.join(flags["output_directory"], flags["target_vocab_path"])), \
        ...
        for split in splits:
            ...
            test_set = GroundedScanDataset(data_path, flags["output_directory"], split=split,
                                           input_vocabulary_file=flags["input_vocab_path"],
                                           target_vocabulary_file=flags["target_vocab_path"], generate_vocabulary=False,
                                           k=flags["k"], simplified_objective=flags["simplified_objective"])
